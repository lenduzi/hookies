...
Styles:
  classic — bold uppercase body text, key words in golden italic at larger size
  pill    — dark rounded-rectangle background, white bold text (3 words per chunk)
"""
...
KEY_FONT_SCALE = 1.18          # key words rendered this much larger (classic only)
FONT_PATH = "/System/Library/Fonts/HelveticaNeue.ttc"
FONT_INDEX_REGULAR = 1         # Helvetica Neue Bold
FONT_INDEX_KEY = 3             # Helvetica Neue Bold Italic
TEXT_COLOR = (255, 255, 255)   # white
KEY_COLOR = (255, 210, 0)      # golden yellow for key words (classic only)
...
# Pill style config
PILL_BG = (15, 15, 15)         # near-black background
PILL_BG_ALPHA = 230            # slightly transparent
PILL_RADIUS = 14               # rounded corner radius
PILL_H_PAD = 22                # horizontal padding inside pill
PILL_V_PAD = 12                # vertical padding inside pill
...
def burn_captions(video_path: str, words: list, output_path: str,
    ...
    Args:
        ...
        style:       "classic" (bold + golden key words) or "pill" (dark bg box)
        key_words:   optional list of strings to highlight in classic style;
    ...
    # Build the active key-word set for classic style
    if key_words:
    ...
    # ── Pill style ────────────────────────────────────────────────────────────
    def make_pill_clip(chunk: dict):
        raw_words = chunk["text"].split()
        # All words use the regular bold font (no key-word differentiation — cleaner look)
        word_fonts = [font_regular for _ in raw_words]

        sizes = [measure_word(w, f) for w, f in zip(raw_words, word_fonts)]
        total_w = sum(s[0] for s in sizes) + WORD_GAP * (len(raw_words) - 1)
        max_h = max(s[1] for s in sizes)

        # Scale down if too wide (account for pill padding)
        available = max_text_width - PILL_H_PAD * 2
        if total_w > available:
            scale = available / total_w
            font_scaled = ImageFont.truetype(FONT_PATH, int(FONT_SIZE * scale), index=FONT_INDEX_REGULAR)
            word_fonts = [font_scaled for _ in raw_words]
            sizes = [measure_word(w, f) for w, f in zip(raw_words, word_fonts)]
            total_w = sum(s[0] for s in sizes) + WORD_GAP * (len(raw_words) - 1)
            max_h = max(s[1] for s in sizes)
        ...
        # White text
        x = PILL_H_PAD
        for word, font, (ww, wh) in zip(raw_words, word_fonts, sizes):
    ...
    # ── Dispatch ──────────────────────────────────────────────────────────────
    make_clip = make_pill_clip if style == "pill" else make_classic_clip

    caption_clips = [make_clip(c) for c in chunks]
